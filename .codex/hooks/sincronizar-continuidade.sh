...
#
# Adaptador (regra de ferro 1): espelha .claude/hooks/sincronizar-continuidade.sh
# para o runtime do Codex. Única diferença: o Codex CLI não expõe uma variável de
# ambiente equivalente a CLAUDE_PROJECT_DIR (não há CODEX_PROJECT_DIR nem análoga
# documentada), então a raiz do repositório é resolvida por `git rev-parse
# --show-toplevel`, que funciona tanto sob Codex quanto sob Claude Code — o
# fallback relativo ao script permanece como última rede de segurança fora de um
# repositório git.
set -euo pipefail
...
elif raiz_git="$(cd "$(dirname "${BASH_SOURCE[0]}")" && git rev-parse --show-toplevel 2>/dev/null)"; then
  raiz="$(cd "$raiz_git" && pwd -P)"
else
